...
`sql-query` 是一个独立的 CLI 工具，专为 AI skills 设计，提供 MySQL 数据库结构查询和 SQL 查询导出能力。工具支持三个核心子命令：查看数据库表列表、查看表 DDL、执行 SQL 并导出为 Excel/HTML/JSON。导出逻辑参考现有 `sql-exporter` 项目的元数据协议、S3 预签名、增强型导出等能力。
...
- **需求**：干净的 stdout 输出（JSON/文本），无干扰日志
...
| `github.com/aws/aws-sdk-go-v2` | S3 预签名 |
...
| `internal/processor/pipeline.go` | `sql-exporter/internal/processor/pipeline.go` | 完整移植，日志改 stderr |
| `internal/exporter/*` | `sql-exporter/internal/exporter/*` | 完整移植 |
| `internal/s3/presigner.go` | `sql-exporter/internal/s3/presigner.go` | 完整移植 |
...
### Step 2: 移植基础模块
- config、db/connect、db/execute、s3、parser、processor
